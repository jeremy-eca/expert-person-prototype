...
export class ReferenceDataService {
   ...
   * Titles API
   */
  static async getTitles(params: SearchParams = {}): Promise<PaginatedResponse<ReferenceTitle>> {
    const cacheKey = this.getCacheKey('titles', params);
    const cached = this.getCachedData<PaginatedResponse<ReferenceTitle>>(cacheKey);
    
    if (cached) return cached;

    try {
      // Use the core API from environment variable
      ...
      const response = await fetch(`${coreApiUrl}/reference_titles?${queryParams}`, {
        method: 'GET',
        headers: {
      ...
      const data = await response.json() as PaginatedResponse<ReferenceTitle>;
      this.setCachedData(cacheKey, data, 10 * 60 * 1000); // 10 minute cache for titles
      return data;
    } catch (error: any) {
      console.error('❌ [REFERENCE_SERVICE] Failed to fetch titles:', error);
      throw new Error(`Unable to fetch titles: ${error.message}`);
    }
  }

  static async getTitle(titleId: string): Promise<ReferenceTitle> {
    const cacheKey = this.getCacheKey('title', { id: titleId });
    const cached = this.getCachedData<ReferenceTitle>(cacheKey);
    
    if (cached) return cached;

    try {
      const response = await getApiClient().get(`/reference_titles/${titleId}`);
      this.setCachedData(cacheKey, response.data, 10 * 60 * 1000); // 10 minute cache
      return response.data;
    } catch (error: any) {
      console.error('❌ [REFERENCE_SERVICE] Failed to fetch title by ID:', error);
      throw new Error(`Unable to fetch title: ${error.message}`);
    }
  }
   ...
   * Genders API
   */
  static async getGenders(params: SearchParams = {}): Promise<PaginatedResponse<ReferenceGender>> {
    const cacheKey = this.getCacheKey('genders', params);
    const cached = this.getCachedData<PaginatedResponse<ReferenceGender>>(cacheKey);
    ...
    try {
      ...
      const timestamp = Date.now().toString();
      const secretKey = import.meta.env.VITE_HMAC_CLIENT_KEY;
      const path = '/reference_gender';
      const stringToSign = path.toLowerCase() + timestamp;
      const signature = crypto.default.HmacSHA256(stringToSign, secretKey).toString(crypto.default.enc.Base64);

      const response = await fetch(`${coreApiUrl}/reference_gender?${queryParams}`, {
        method: 'GET',
        headers: {
          'x-client-id': import.meta.env.VITE_HMAC_CLIENT_ID,
          'x-signature': signature,
      ...
      const data = await response.json() as PaginatedResponse<ReferenceGender>;
      this.setCachedData(cacheKey, data, 10 * 60 * 1000); // 10 minute cache for genders
      return data;
    } catch (error: any) {
      console.error('❌ [REFERENCE_SERVICE] Failed to fetch genders:', error);
      throw new Error(`Unable to fetch genders: ${error.message}`);
    }
  }

  static async getGender(genderId: string): Promise<ReferenceGender> {
    const cacheKey = this.getCacheKey('gender', { id: genderId });
    const cached = this.getCachedData<ReferenceGender>(cacheKey);
    
    if (cached) return cached;

    try {
      const response = await getApiClient().get(`/reference_gender/${genderId}`);
      this.setCachedData(cacheKey, response.data, 10 * 60 * 1000); // 10 minute cache
      return response.data;
    } catch (error: any) {
      console.error('❌ [REFERENCE_SERVICE] Failed to fetch gender by ID:', error);
      throw new Error(`Unable to fetch gender: ${error.message}`);
    }
  }
