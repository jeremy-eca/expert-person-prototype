...
import { ProfileSectionType } from "../../ProfilesEdit";
import { PersonProfile, LanguageSkill } from "../../../../types/frontend.types";
import { FieldMetadata, PersonProfileCreateRequest } from "../../../../types/api.types";
...
export const ProfileSection = ({ 
  ...
  const [isEditingPersonal, setIsEditingPersonal] = useState(isNewPerson); // Auto-edit for new persons
  ...
  const [selectedTitleId, setSelectedTitleId] = useState<string>("no-title"); // Store title_id, not the display value
  const [selectedGenderId, setSelectedGenderId] = useState<string>("no-gender"); // Store gender_id, not the display value
  ...
  const validateNewPerson = () => {
    ...
    if (contactForm.workEmail.trim()) {
      ...
      if (!emailRegex.test(contactForm.workEmail.trim())) {
        errors.work_email = 'Please enter a valid work email address';
      }
    }

    if (contactForm.personalEmail.trim()) {
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(contactForm.personalEmail.trim())) {
  ...
  // Save new person profile
  const handleSaveNewPerson = async () => {
    console.log('🚀 [STEP 1] handleSaveNewPerson called');
    console.log('🔍 [STEP 1] isNewPerson:', isNewPerson);
    console.log('🔍 [STEP 1] onProfileCreate exists:', !!onProfileCreate);
    
    if (!isNewPerson || !onProfileCreate) {
      console.log('❌ [STEP 1] Early return - not new person or no onProfileCreate');
      return;
    }
    ...
    console.log('🔍 [STEP 2] Form state BEFORE validation:');
    console.log('  - editFirstName:', `"${editFirstName}"`);
    console.log('  - editLastName:', `"${editLastName}"`);
    console.log('  - editBio:', `"${editBio}"`);
    console.log('  - editDateOfBirth:', `"${editDateOfBirth}"`);
    console.log('  - editNationalities:', `"${editNationalities}"`);
    console.log('  - selectedTitleId:', `"${selectedTitleId}"`);
    console.log('  - selectedGenderId:', `"${selectedGenderId}"`);
    console.log('  - contactForm:', contactForm);
    console.log('  - editCurrentAddress:', editCurrentAddress);
    
    // Validate required fields
    const validationResult = validateNewPerson();
    console.log('🔍 [STEP 3] Validation result:', validationResult);
    console.log('🔍 [STEP 3] Validation errors:', validationErrors);
    
    if (!validationResult) {
      const errorCount = Object.keys(validationErrors).length;
      console.log('❌ [STEP 3] Validation failed, error count:', errorCount);
      if (errorCount === 1) {
    ...
    try {
      ...
      console.log('🔍 [STEP 4] Starting profile data building...');

      // Build the profile creation request
      console.log('🔍 [STEP 5] Building base profileData with individual field processing:');
      
      const firstName = editFirstName.trim();
      const lastName = editLastName.trim();
      const bio = editBio.trim() || undefined;
      const dateOfBirth = editDateOfBirth || undefined;
      const email = contactForm.workEmail.trim() || undefined;
      
      console.log('  - Processing firstName:', `"${editFirstName}" -> "${firstName}"`);
      console.log('  - Processing lastName:', `"${editLastName}" -> "${lastName}"`);
      console.log('  - Processing bio:', `"${editBio}" -> "${bio}"`);
      console.log('  - Processing dateOfBirth:', `"${editDateOfBirth}" -> "${dateOfBirth}"`);
      console.log('  - Processing email:', `"${contactForm.workEmail}" -> "${email}"`);
      
      const profileData: PersonProfileCreateRequest = {
        first_name: firstName,
        last_name: lastName,
        preferred_name: firstName, // Use first name as preferred name by default
        bio: bio,
        date_of_birth: dateOfBirth,
        email: email, // Use contact form state
        // Add required employee_reference field with correct JSONB structure
        employee_reference: {
          primary: {
            id: `${firstName}${lastName}${Date.now()}`.replace(/\s+/g, ''),
            source: "WEB_FORM",
            type: "manual_entry"
          },
          references: [
      ...
      console.log('🔍 [STEP 6] Base profileData created:', JSON.stringify(profileData, null, 2));

      console.log('🔍 [STEP 7] Processing optional fields...');
      
      // Only include title_id if a title was actually selected (not "no-title")
      console.log('  - Checking title selection:', `"${selectedTitleId}"`);
      if (selectedTitleId && selectedTitleId.trim() !== '' && selectedTitleId !== 'no-title') {
        profileData.title_id = selectedTitleId;
        console.log('  ✅ Added title_id:', selectedTitleId);
      } else {
        console.log('  ⏭️ Skipping title_id (not selected or no-title)');
      }

      // Only include gender_id if a gender was actually selected (not "no-gender")
      console.log('  - Checking gender selection:', `"${selectedGenderId}"`);
      if (selectedGenderId && selectedGenderId.trim() !== '' && selectedGenderId !== 'no-gender') {
        profileData.gender_id = selectedGenderId;
        console.log('  ✅ Added gender_id:', selectedGenderId);
      } else {
        console.log('  ⏭️ Skipping gender_id (not selected or no-gender)');
      }

      // Add nationalities from form state
      console.log('  - Checking nationalities:', `"${editNationalities}"`);
      if (editNationalities.trim()) {
        const nationalitiesArray = editNationalities
          .split(',')
          .map(nat => nat.trim())
          .filter(Boolean);
        profileData.nationalities = nationalitiesArray;
        console.log('  ✅ Added nationalities:', nationalitiesArray);
      } else {
        console.log('  ⏭️ Skipping nationalities (empty)');
      }
      
      // Add address from form state if any address fields are filled
      if (editCurrentAddress.line1.trim() || editCurrentAddress.city.trim()) {
        profileData.address = {
          name: 'Current Address',
          line1: editCurrentAddress.line1.trim() || undefined,
          line2: editCurrentAddress.line2.trim() || undefined,
          city: editCurrentAddress.city.trim() || undefined,
          state: editCurrentAddress.state.trim() || undefined,
          postcode: editCurrentAddress.postcode.trim() || undefined,
          country: editCurrentAddress.country.trim() || undefined
        };
        console.log('🔍 [DEBUG] Added address:', profileData.address);
      }
      
      // Add contact details from form state if any contact fields are filled
      if (contactForm.workEmail.trim() || contactForm.personalEmail.trim() || 
          contactForm.workPhone.trim() || contactForm.mobilePhone.trim()) {
        profileData.contact = {
          work_email_address: contactForm.workEmail.trim() || undefined,
          work_phone_number: contactForm.workPhone.trim() || undefined,
          personal_email_address: contactForm.personalEmail.trim() || undefined,
          mobile_number: contactForm.mobilePhone.trim() || undefined
        };
        console.log('🔍 [DEBUG] Added contact:', profileData.contact);
      }
      
      console.log('🚀 [DEBUG] Final profileData being sent to API:', JSON.stringify(profileData, null, 2));
      
      // Create the person via PersonService
      const createdPersonId = await onProfileCreate(profileData);
      
      toast.success(`Person "${editFirstName} ${editLastName}" created successfully!`, { id: 'create-person' });
      
      // Notify parent component
      if (onPersonCreated) {
        onPersonCreated(createdPersonId);
      }
      
    } catch (error: any) {
      console.error('❌ [PROFILE_SECTION] Failed to create person:', error);
      const errorMessage = error.message || 'Failed to create person profile';
      setSaveError(errorMessage);
      toast.error(`Failed to create person: ${errorMessage}`, { id: 'create-person' });
    } finally {
      setIsSaving(false);
    }
  };
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (validationErrors.work_email) {
                        setValidationErrors(prev => ({ ...prev, work_email: '' }));
                      }
                    }}
                    className={`bg-[#1D252D] border-[#40505C] text-white ${
                      validationErrors.work_email ? 'border-red-400' : ''
                    }`}
                    placeholder="work@company.com"
                  />
                  {validationErrors.work_email && (
                    <p className="text-red-400 text-sm mt-1">{validationErrors.work_email}</p>
                  )}
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      setContactForm(prev => ({ ...prev, personalEmail: e.target.value }));
                      // Clear validation error when user starts typing
                      if (validationErrors.personal_email) {
                        setValidationErrors(prev => ({ ...prev, personal_email: '' }));
                      }
                    }}
                    className={`bg-[#1D252D] border-[#40505C] text-white ${
                      validationErrors.personal_email ? 'border-red-400' : ''
                    }`}
                    placeholder="personal@email.com"
                  />
                  {validationErrors.personal_email && (
                    <p className="text-red-400 text-sm mt-1">{validationErrors.personal_email}</p>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-7 space-y-6">
          ...
          <Card className="bg-[#252E38] border-0 rounded-xl shadow-[0px_6px_18px_0px_#00000026]">
            ...
            <CardContent className="p-6 pt-0">
              {/* Instructions for new person creation */}
              {isNewPerson && !isEditingPersonal && (
                <div className="mb-4 p-3 bg-blue-900/20 border border-blue-400 rounded-md">
                  <p className="text-blue-400 text-sm">
                    Click the edit button to start creating a new person profile. First name and last name are required.
                  </p>
                </div>
              )}
              {/* Error display for new person creation */}
              {saveError && isNewPerson && (
                <div className="mb-4 p-3 bg-red-900/20 border border-red-400 rounded-md">
              ...
              {!isEditingPersonal ? (
                <div className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white">
                      {selectedTitleId && selectedTitleId !== '' && selectedTitleId !== 'no-title' ? 
                        titles.find(t => t.id === selectedTitleId)?.display_label || 'Unknown Title'
                        : 'No title'
                      }
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-white">
                      {selectedGenderId && selectedGenderId !== '' && selectedGenderId !== 'no-gender' ? 
                        genders.find(g => g.id === selectedGenderId)?.display_label || 'Unknown Gender'
                        : 'Not specified'
                      }
                    </span>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    >
                      <SelectTrigger className="bg-[#2A3440] border-[#40505C] text-white placeholder:text-gray-400">
                        <SelectValue placeholder={titlesLoading ? "Loading titles..." : "Select a title"} />
                      </SelectTrigger>
                      <SelectContent className="bg-[#2A3440] border-[#40505C] text-white">
                        <SelectItem 
                          value="no-title"
                          className="text-white hover:bg-[#40505C] focus:bg-[#40505C]"
                        >
                          No title
                        </SelectItem>
                        {titles.map((title) => (
                  ...
                  <div>
                    <label className="text-white text-sm font-medium mb-2 block">
                      First name{isNewPerson && <span className="text-red-400 ml-1">*</span>}
                    </label>
                    <Input
                      value={editFirstName}
                      onChange={(e) => {
                      ...
                      className={`bg-[#2A3440] border-[#40505C] text-white placeholder:text-gray-400 ${
                        validationErrors.first_name ? 'border-red-400' : ''
                      }`}
                    />
                  ...
                  <div>
                    <label className="text-white text-sm font-medium mb-2 block">
                      Last name{isNewPerson && <span className="text-red-400 ml-1">*</span>}
                    </label>
                    <Input
                      value={editLastName}
                      onChange={(e) => {
                      ...
                      className={`bg-[#2A3440] border-[#40505C] text-white placeholder:text-gray-400 ${
                        validationErrors.last_name ? 'border-red-400' : ''
                      }`}
                    />
                  ...
                  <div>
                    ...
                    >
                      <SelectTrigger className="bg-[#2A3440] border-[#40505C] text-white placeholder:text-gray-400">
                        <SelectValue placeholder={gendersLoading ? "Loading genders..." : "Select gender at birth"} />
                      </SelectTrigger>
                      <SelectContent className="bg-[#2A3440] border-[#40505C] text-white">
                        <SelectItem 
                          value="no-gender"
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      className={`bg-[#2A3440] border-[#40505C] text-white placeholder:text-gray-400 ${
                        validationErrors.date_of_birth ? 'border-red-400' : ''
                      }`}
                    />
                    {validationErrors.date_of_birth && (
                      <p className="text-red-400 text-sm mt-1">{validationErrors.date_of_birth}</p>
                    )}
                  </div>
                </div>
              )}
  ...
  // Card-based details section with React Hook Form
  const renderDetailsWithCards = () => {
    // Don't render the form until we have profile data (unless creating a new person)
    if (!profile && !isNewPerson) {
      return (
        <div className="flex-1 p-8 bg-[#1D252D] overflow-y-auto">
    ...
    const profileForm = useProfilesEditForm({
      profile,
      onProfileUpdate,
      onProfileCreate,
      onPersonCreated,
    ...
    // Add error handling in case the form is not properly initialized
    if (!profileForm?.form) {
      return (
        <div className="flex-1 p-8 bg-[#1D252D] overflow-y-auto">
          <div className="flex items-center justify-center h-64">
            <p className="text-white">Initializing form...</p>
          </div>
        </div>
      );
    }
    ...
    return (
      ...
      {/* Card-based Layout with React Hook Form */}
      <div className="grid grid-cols-12 gap-6">
        {/* Left Column - Wider (Personal and Bio Cards) */}
        <div className="col-span-7 space-y-6">
          {/* Personal Card */}
          <PersonalCard
            profile={profile}
