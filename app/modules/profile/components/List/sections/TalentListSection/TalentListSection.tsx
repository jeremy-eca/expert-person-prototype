...
import { getProfileDataService } from "../../../../services/ProfileDataService";
import { mapPersonToListItem } from "../../../../utils/personMapper";
import { PersonListItem } from "../../../../types/frontend.types";
import { getMockListData } from "../../getMockListData";
...
export const TalentListSection = ({ onPersonSelect }: TalentListSectionProps): JSX.Element => {
  ...
  const fetchPeople = useCallback(async () => {
    try {
      ...
      const mappedPeople = result.persons.map(person => 
        mapPersonToListItem(person)
      );
      ...
      setError('Using demo data - API connection not available. Please check your connection.');
      // Use mock data as fallback to show perfect grid
      const mockPeople = getMockListData();
      setPeople(mockPeople);
      setTotalPeople(mockPeople.length);
    } finally {
      setLoading(false);
    }
  ...
  const handleDeletePerson = async (e: React.MouseEvent, personId: string, personName: string) => {
    ...
    if (confirm(`Are you sure you want to delete ${personName}?`)) {
      try {
        const profileDataService = getProfileDataService();
        await profileDataService.deletePersonProfile(personId);
        // Refresh the list
        fetchPeople();
      } catch (err) {
        ...
        {!loading && !error && people.length > 0 && (
          <div className="w-full p-3 bg-green-50 border border-green-200 rounded-lg mb-4">
            <p className="text-sm text-green-800">
              ✅ Successfully connected to Expert Persons API - Showing {totalPeople} real persons
            </p>
            <p className="text-xs text-green-700 mt-1">
              Click any person to edit • Click "Add" button to create new person
            </p>
          </div>
        )}
        
        {error && (
          <div className="w-full p-3 bg-orange-50 border border-orange-200 rounded-lg mb-4">
            <p className="text-sm text-orange-800">
              🔧 {error}
            </p>
            <p className="text-xs text-orange-700 mt-1">
              Showing {totalPeople} demo records • All features work exactly the same
            </p>
          </div>
        )}
